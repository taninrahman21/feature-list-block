...
const UseInlineFeatureLists = compose(withSelect((select) => { return { device: select("core/edit-post").__experimentalGetPreviewDeviceType()?.toLowerCase() } }))(({ attributes, setAttributes, device }) => {
  ...
  return (
    <div>
      <ToggleControl
        label={__("Use Inline Features", "feature-list")}
      ...
      {/* Items Per Line */}
      {
        ...
        <>
          <div>
            <PanelRow>
              <Label className='mb5'>{__("Space Between Item", 'b-blocks')}</Label>
              <Device />
            </PanelRow>
            <RangeControl
              label={__("Items Per Line", "feature-list")}
          ...
          <div>
            ...
            <RangeControl
              label={__("Gap Between Item", "feature-list")}
