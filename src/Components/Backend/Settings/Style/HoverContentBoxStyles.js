...
const HoverContentBoxStyles = compose(withSelect((select) => { return { device: select("core/edit-post").__experimentalGetPreviewDeviceType()?.toLowerCase() } }))(({ attributes, setAttributes, device }) => {
  ...
  return (
    <div>
      ...
      <RangeControl
        label={__("Background Transition", "b-blocks")}
...

      {
        contentBox.hoverBorder.style !== "none" && (
          <>
            ...
            {/* Border Transition  */}
            <RangeControl
              label={__("Border Transition", "b-blocks")}
      ...
      <RangeControl
        label={__("Border Radius Transition", "b-blocks")}
      ...
      <RangeControl
        label={__("Box Shadow Transition", "b-blocks")}
